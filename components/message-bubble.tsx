...
const MessageBubble = ({ id, hidden }: MessageBubbleProps) => {
  return (
    <div
      id={id}
      role="tooltip"
      className={`animate-fly-in-up sm:animate-fly-in-down opacity-0 bg-gray-800 text-white absolute py-2 px-3 whitespace-nowrap -top-1/3 left-1/2 transform -translate-x-1/2 text-center rounded-lg sm:top-full sm:mt-4 ${
        hidden ? "hidden" : ""
      }`}
      aria-hidden={hidden}
    >
