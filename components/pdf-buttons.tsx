...
function PdfButtons({ document, fileName, className }: PdfButtonsProps) {
    const [{ url, loading }] = usePDF({ document });
    ...
    return (
        <div className={classNames("sm:flex max-w-[50vh] px-4 sm:max-w-none sm:gap-3 mx-auto space-y-3 sm:space-y-0 justify-center items-center", className)}>
            <a
                download={fileName}
                href={url}
                className="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3 border border-transparent shadow-sm text-lg font-medium rounded-full text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600"
            >
