    ...
    return (
        <ul className="sm:grid s:grid-flow-row sm:auto-rows-fr sm:justify-center sm:gap-12 sm:min-h-[256px] 3xl:mx-auto 3xl:my-0 3xl:grid-cols-projects">
            {projects.map((project) => (
                <li
                    key={project.title}
                    className="transition-all duration-100 ease-in mt-16 sm:flex sm:items-center sm:pl-5 sm:mt-0 md:px-5 md:max-w-[800px] md:min-h-[250px]"
                >
                    ...
                    >
                        <Image src={"https:" + project.banner.fields.file.url + "?w=600"} alt={project.title} layout="fill" objectFit="cover" />
