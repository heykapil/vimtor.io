    ...
    return (
        <li className="transition-all duration-100 ease-in mt-16 sm:flex sm:items-center sm:pl-5 sm:mt-0 md:px-5 md:max-w-[800px] md:min-h-[250px]">
            <div className="max-w-[90%] m-0 mx-auto w-full sm:w-1/2 sm:mr-8 sm:text-left md:mr-12">
                <h3 className="mb-3 font-bold text-2xl">{props.project.title}</h3>
                <RichText>{props.project.content}</RichText>
                <div className="flex justify-center items-center mt-4 space-x-3 sm:justify-start">
                    <Link href={props.project.source}>{props.project.ctaMessage}</Link>
                    {props.project.githubRepositoryUrl ? (
                        <Link href={props.project.githubRepositoryUrl}>
            ...
            <a
                ...
                aria-label={`${props.project.title} source`}
            >
                <Image src={"https:" + props.project.banner.fields.file.url + "?w=600"} alt={props.project.title} layout="fill" objectFit="cover" />
